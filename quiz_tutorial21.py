...
def create_textinputquiz_widget(description, text_description, correct_answer, a2, hint):  ##grid for option table
    ...
    def check_selection(b):
        ...
        if a == correct_answer:
            s = "You are right! The third level 2 magic word is 'A'"

        elif a == "JOHN HATES BROCOLLI" :
            s = "cheat code : sum(GRID_EXPORT.p.get_value())"

        else:
            s = "It seems like you got the wrong result. Do you have the magic words from Tutorial 1? try it!"
...
def quiz():
    questions = create_textinputquiz_widget('How much energy was exported to the grid before in Tutorial 1?\n (one decimal)', "answer:", '1.2', "kWh",
                                            '[hint]:Visit Tutorial 1 chapter 1.10')
